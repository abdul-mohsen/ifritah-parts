...
func TestIsHKOEM_GoldenCases(t *testing.T) {
	golden := []struct {
		...
		{"26300-35505", true},   // Hyundai/Kia oil filter ΓÇö golden set
		{"97133-D3000", true},   // Hyundai/Kia Tucson cabin filter ΓÇö golden set
		...
		{"26300 35505", true},   // With whitespace ΓÇö should still match dashed after compact
...
func TestIsHKOEM_SuggestsMake(t *testing.T) {
	...
	// the user which make it looks like ΓÇö the honest "not in scope" message.
	cases := []struct {
		oem   string
		make  string
	}{
...
func TestIsJunkDescription(t *testing.T) {
	...
	legit := []string{
		...
		"Timing chain kit ΓÇö engine 1.6 T-GDI",
	}
	for _, l := range legit {
