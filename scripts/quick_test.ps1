# Quick regression - save results to file
...
try {
    ...
    $wc.DownloadString("http://localhost:8080/health") | Out-Null
} catch {
    "Server not responding" | Out-File .\test_regression.txt
    exit 1
}
...
foreach ($t in $tests) {
    ...
    try {
        ...
        $raw = $data.nhtsaRaw
        if ($raw.make -eq $expMake -and $raw.modelYear -eq $expYear) { $pass++ }
        else { $fail++; $failList += "FAIL $vin exp=$expMake/$expYear got=$($raw.make)/$($raw.modelYear)" }
    } catch { $fail++; $failList += "ERROR $vin $_" }
}

$result = "Results: $pass passed, $fail failed out of $($tests.Count)"
$result | Out-File .\test_regression.txt
if ($failList.Count -gt 0) {
    $failList | Out-File .\test_regression.txt -Append
}
